...
class Ebulb_Newreports_Block_Adminhtml_Salesreport_Salesperson_Filter_Form extends Mage_Adminhtml_Block_Widget_Form
{
    /**
     * Report type options
     */
    protected $_reportTypeOptions = array();
    protected $_salespersonoptions = array();
    ...
    protected function _prepareForm()
    {
       ...
       $statusfield->setRenderer($this->getLayout()->createBlock('newreports/adminhtml_renderer_status'));
        ...
        $this->_salespersonoptions['all']      = 'All';
        $this->_salespersonoptions['Amazon']   = 'Amazon';  
        $this->_salespersonoptions['eBay']     = 'eBay';  
        
        //$roleId = 13;  // customer service department id
        //$users  = Mage::getModel('admin/roles')->setId($roleId)->getRoleUsers();
        $collection = Mage::getResourceModel('admin/user_collection'); 
        
        //$collection = Mage::getResourceModel('admin/user_collection')  
        //              ->addFieldToFilter("user_id not in (".implode(",",$users).")")
        //              ->load(); 
                      
       
        foreach($collection as $userdata){
            $this->_salespersonoptions[$userdata->getData('username')] = $userdata->getData('username');//." ".$userdata->getData('lastname');   
        }
       
        $fieldset->addField('sales_person', 'select', array(
            'name'      => 'sales_person',
            'options'   => $this->_salespersonoptions,
            'label'     => Mage::helper('reports')->__('Sales Person'),
            'title'     => Mage::helper('reports')->__('Sales Person')
        ));  
