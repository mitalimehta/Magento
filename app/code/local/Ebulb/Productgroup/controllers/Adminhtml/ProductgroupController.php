...
class Ebulb_Productgroup_Adminhtml_ProductgroupController extends Mage_Adminhtml_Controller_action
{ 
	...
	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu('productgroup/items')
	...
	public function editAction() {
		...
		$model  = Mage::getModel('productgroup/productgroup')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			Mage::register('productgroup_data', $model);

			$this->loadLayout();
			$this->_setActiveMenu('productgroup/items');
			...
			$this->_addContent($this->getLayout()->createBlock('productgroup/adminhtml_productgroup_edit'))
				->_addLeft($this->getLayout()->createBlock('productgroup/adminhtml_productgroup_edit_tabs'));

			$this->renderLayout();
		} else {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('productgroup')->__('Item does not exist'));
			$this->_redirect('*/*/');
		}
	}
    ...
    public function productskuAction() {
        ...
        if ($model->getId() || $id == 0) {
            ...
            $this->_addContent($this->getLayout()->createBlock('productgroup/adminhtml_productgroup_productsku'))
                ->_addLeft($this->getLayout()->createBlock('productgroup/adminhtml_productgroup_productsku_tabs'));
	...
	public function saveAction() {
		...
		/*	if(isset($_FILES['filename']['name']) && $_FILES['filename']['name'] != '') {
			...
			$model = Mage::getModel('productgroup/productgroup');		
			$model->setData($data)
    ...
    public function massDeleteAction() {
        ...
        $groupIds = $this->getRequest()->getParam('productgroup');
        if(!is_array($groupIds)) { 
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select item(s)'));
        } else { 
            try {
                foreach ($groupIds as $groupId) {
                    $group = Mage::getModel('productgroup/productgroup')->load($groupId);
                    $group->delete();
                }
...
//        $this->_redirect('*/*/index');
 //   }
    ...
    public function exportCsvAction()
    {
        ...
        $content    = $this->getLayout()->createBlock('productgroup/adminhtml_productgroup_grid')
            ->getCsv();
