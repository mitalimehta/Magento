...
class Ebulb_Purchase_Model_Pdf_Purchaseorder extends Ebulb_Purchase_Model_Pdf_Pdfhelper
{
    ...
    public function getPdf($orders = array())
    {
        ...
        foreach ($orders as $order) 
        {
            ...
            if ($order->getStatus() != Ebulb_Purchase_Model_Order::STATUS_INQUIRY )
            {
            ...
            $this->y -= 20;
            $page->drawText(Mage::helper('purchase')->__('Shipping Method : %s', $order->getShippingType()), 15, $this->y, 'UTF-8');
            //$order->getpo_carrier()
     ...
     public function drawTableItems(&$page,$order,&$style,&$settings)
     {
            ...
            foreach ($order->getOrderItems() as $item)
            {
                ...
                if ($order->getStatus() != Ebulb_Purchase_Model_Order::STATUS_INQUIRY )
                {
                    $this->drawTextInBlock($page, number_format($item->getpop_tax_rate(), 2).'%', 440, $this->y, 40, 20, 'c');     
                    $this->drawTextInBlock($page, $order->getCurrency()->formatTxt($item->getRowSubtotal()), 465, $this->y, 60, 20, 'r');     
                    $this->drawTextInBlock($page, $order->getCurrency()->formatTxt($item->getRowTotal()), 520, $this->y, 60, 20, 'r');   
                }
